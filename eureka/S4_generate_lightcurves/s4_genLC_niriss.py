...
def genlc(eventlabel, ecf_path=None, s3_meta=None):
    ...
    meta.filename_S4_SpecData = (meta.outputdir + 'S4_' + meta.eventlabel
                                 + "_SpecData.h5")
    xrio.writeXR(meta.filename_S4_SpecData, spec, verbose=True)
    # Save Dataset object containing binned light curves
    meta.filename_S4_LCData = (meta.outputdir + 'S4_' + meta.eventlabel
                               + "_LCData.h5")
...
def load_specific_s3_meta_info(meta):
    ...
    inputdir = os.sep.join(meta.inputdir.split(os.sep)[:-1])
