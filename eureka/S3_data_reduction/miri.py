...
#     np.savetxt('./lam_x.txt', (lam_x))
#     # Including nans for out of BB area (eg for reference pixels) so that length agrees with detector/subarray size
#     lam_x_full = np.array([np.float64(np.nan)] * int(y[0, 0]) + lam_x + [np.float64(np.nan)] * int(416 - y[-1, 0] - 1))
#     np.savetxt('./lam_x_full.txt', (lam_x_full))
#     print(lam_x_full.tolist())
#
#
#     return lam_x_full
