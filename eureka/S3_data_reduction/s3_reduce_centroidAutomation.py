...
from photutils.centroids import (centroid_2dg)  # RA 2023
# from photutils.centroids import (centroid_1dg, centroid_2dg, centroid_com, centroid_quadratic)  # RA 2023
...
def reduce(eventlabel, ecf_path=None, s2_meta=None, input_meta=None):
    ...
    # # convert GA input to integer
    # meta.bg_hw = int(meta.bg_hw[0])    #Reza  RA2022
    ...
    for spec_hw_val in meta.spec_hw_range:
        for bg_hw_val in meta.bg_hw_range:
            ...
            log.writelog(f"Using ap={spec_hw_val}, bg={bg_hw_val}, p3thresh={meta.p3thresh}, median_thresh={meta.median_thresh}, p5thresh={meta.p5thresh}, p7thresh={meta.p7thresh}")
            ...
            for m in range(meta.nbatch):
                ...
                if not meta.photometry:
                    ...
                    for i in tqdm(range(len(data.time)),
                        ...
                        ## Automated centroid locating - RA 2023
                        # from photutils.centroids import (centroid_2dg)
                        # position = centroid_1dg(data)  # RA 2023
                        position = centroid_2dg(data)  # RA 2023
                        # position = centroid_quadratic(data)  # RA 2023
                        # position = centroid_com(data)  # RA 2023
