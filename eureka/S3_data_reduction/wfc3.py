...
def preparation_step(meta, log):
    ...
    # Initialize lists to hold important parameters for later steps in this stage
    meta.centroids = []
    meta.subflat = []
    meta.flatmask = []
    meta.scanHeight = []
...
def read(filename, data, meta):
    ...
    with fits.open(filename) as hdulist:
        ...
        meta.flatoffset         = [[-1*data.attrs['shdr']['LTV2'], -1*data.attrs['shdr']['LTV1']]]
    ...
    frametime = 2400000.5 + 0.5*(data.attrs['mhdr']['EXPSTART'] + data.attrs['mhdr']['EXPEND'])
    if meta.horizonsfile != None:
        ...
        time = utc_tt.utc_tdb(bjdutc, meta.leapdir)   # FINDME: this was utc_tt, but I believe it should have been utc_tdb instead
    ...
    wave = hst.calibrateLambda(xrange, centroids[0], meta.grism)/1e4   #wavelength in microns
    wave_2d = wave*np.ones((meta.ny,1)) # Assume no skew over the detector
    wave_units = 'microns'
...
def flatfield(data, meta):
    ...
    print(meta.flatfile)
    tempflat, tempmask = hst.makeflats(meta.flatfile, [np.mean(data.wave_2d.values,axis=0),], [[0,meta.nx],], [[0,meta.ny],], meta.flatoffset, 1, meta.ny, meta.nx, sigma=meta.flatsigma, isplots=meta.isplots_S3)
    subflat  = tempflat[0]
    flatmask = tempmask[0]
...
def difference_frames(data, meta):
    ...
    for n in range(meta.nreads-1):
        ...
        guess[n]  = np.median(np.where(masked_data > np.mean(masked_data))[0]).astype(int)
    ...
    diffdata['flux'] = xrio.makeFluxLikeDA(diffflux, difftime, flux_units, time_units, name='flux')
    diffdata['err']  = xrio.makeFluxLikeDA( differr, difftime, flux_units, time_units, name='err')
    diffdata['mask'] = xrio.makeFluxLikeDA(diffmask, difftime,     "None", time_units, name='mask')
    variance = np.zeros_like(diffdata.flux.values)
    diffdata['variance'] = xrio.makeFluxLikeDA(variance, difftime, flux_units, time_units, name='variance')
...
def correct_drift2D(data, meta, m):
    ...
    if meta.ncpu == 1:
        ...
        for n in range(meta.nreads-1):
            writeDrift2D(hst.calcDrift2D(meta.subdata_ref[p][0]*meta.subdiffmask[p][0],
                                         data.flux[n]*meta.subdiffmask[-1][n], m, n, meta.num_data_files))
    else:
        # Multiple CPUs
        ...
        for n in range(meta.nreads-1):
            res = pool.apply_async(hst.calcDrift2D, args=(meta.subdata_ref[p][0]*meta.subdiffmask[p][0],
                                                          data.flux[n]*meta.subdiffmask[-1][n],
    ...
    for n in range(meta.nreads-1):
        data.flux[n]     = spni.shift(data.flux[n],  -1*meta.drift2D_int[-1][n,::-1], order=0,
                                         mode='constant', cval=0)
        data.mask[n]     = spni.shift(data.mask[n],  -1*meta.drift2D_int[-1][n,::-1], order=0,
                                         mode='constant', cval=0)
        data.variance[n] = spni.shift(data.variance[n], -1*meta.drift2D_int[-1][n,::-1], order=0,
                                         mode='constant', cval=0)
        data.bg[n]       = spni.shift(data.bg[n],    -1*meta.drift2D_int[-1][n,::-1], order=0,
                                         mode='constant', cval=0)
    ...
    for n in range(meta.nreads-1):
        ...
        spline = spi.RectBivariateSpline(iy, ix, data.flux[n], kx=kx, ky=ky, s=0)
        ...
        data.flux[n]  = spline((iy-meta.drift2D[-1][n,1]+meta.drift2D_int[-1][n,1]).flatten(),
                                  (ix-meta.drift2D[-1][n,0]+meta.drift2D_int[-1][n,0]).flatten())
        spline = spi.RectBivariateSpline(iy, ix, data.mask[n], kx=kx, ky=ky, s=0)
        data.mask[n]  = spline((iy-meta.drift2D[-1][n,1]+meta.drift2D_int[-1][n,1]).flatten(),
                                  (ix-meta.drift2D[-1][n,0]+meta.drift2D_int[-1][n,0]).flatten())
        ...
        data.variance[n] = spline((iy-meta.drift2D[-1][n,1]+meta.drift2D_int[-1][n,1]).flatten(),
                                     (ix-meta.drift2D[-1][n,0]+meta.drift2D_int[-1][n,0]).flatten())
        spline = spi.RectBivariateSpline(iy, ix, data.bg[n], kx=kx, ky=ky, s=0)
        data.bg[n]    = spline((iy-meta.drift2D[-1][n,1]+meta.drift2D_int[-1][n,1]).flatten(),
                                  (ix-meta.drift2D[-1][n,0]+meta.drift2D_int[-1][n,0]).flatten())
