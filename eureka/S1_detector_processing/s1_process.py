...
class EurekaS1Pipeline(Detector1Pipeline):
    ...
    def run_eurekaS1(self, filename, meta, log):
        ...
        if hasattr(meta, 'jump_rejection_threshold') and isinstance(meta.jump_rejection_threshold, float):
            self.jump.rejection_threshold = meta.jump_rejection_threshold
        else:
            log.writelog("\nJump rejection threshold is not defined or not a float, default is used (4.0)")
