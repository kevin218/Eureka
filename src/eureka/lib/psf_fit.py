...
def make_psf_interp(spsf, shape, scale, params, psfctr, *args):
  ...
  lims = np.array([np.around(psfctr) - np.around(psf_shape/2),
                   np.around(psfctr) - np.around(psf_shape/2) + psf_shape]).astype(int)
  #print(lims)
  #print('\n')
  #print(lims[0,0],lims[1,0])
  #print(lims[0,1],lims[1,1])
  #print(spsf.shape)
  #print(spsf[lims[0,0]:lims[1,0], lims[0,1]:lims[1,1]])
...
def psf_fit(data, fluxguess, spsf, psfctr, scale, shift, make="bpf",
  ...
  while(step > 0):
    ...
    for shifty in np.arange(-1,2):
      ...
      #print('pos[0]+shifty*step', pos[0],shifty,step)
      ...
      for shiftx in np.arange(-1,2):
        ...
        if chisq[posy, posx] == -1:
          ...
          #print('int(y[posy]), int(x[posx])', int(y[posy]), int(x[posx]))
          model, center = maker(spsf, shape, scale,
                                [y[posy], x[posx], int(1.0), int(0.0)], psfctr, subpsf)
          #print('center',center)
  ...
  model, center = maker(spsf, shape, scale, [y[pos[0]], x[pos[1]], 1, 0],
                        psfctr, subpsf)
...
def spitzer_fit(data, mask, weights, psf, psfctr, scale, make,
  ...
  shift = (yshift, xshift)
  #print('shift', shift)
  #print(data, fluxguess, psf, psfctr, scale,shift, mask, weights, make)
  #import sys
  #sys.exit()
  # Do the fit:
  #print(len(shift))
  pos, bestp, chisq, err = psf_fit(data, fluxguess, psf, psfctr, scale,
                              shift, mask=mask, weights=weights, make=make)

  # Return best fit: [y, x, starflux, skyflux]
  return (pos[0], pos[1], bestp[0], bestp[1], err[0], err[1])
