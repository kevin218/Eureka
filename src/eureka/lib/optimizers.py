...
def parametric_sweep_S1(objective_function, bounds_var, eventlabel,
                        s1_meta_GA, s2_meta_GA, s3_meta_GA, s4_meta_GA, 
    ...
    for var1 in range(bounds_var[0], bounds_var[1] + 1):
        ...
        try:
            x = np.array([var1])
            fitness_value = objective_function(x, eventlabel,
                                               s1_meta_GA, s2_meta_GA,
...
# Parametric sweep function
def parametric_sweep_S3(objective_function, bounds_var, eventlabel,
                        last_s2_meta_outputdir, s3_meta_GA, s4_meta_GA,
                        scaling_MAD_white, scaling_MAD_spec):
    """
    Eureka! Optimization Tools: Parametric Sweep
...
# Parametric sweep function
def parametric_sweep_p7thresh_S3(objective_function, bounds_var, eventlabel,
                                 last_s2_meta_outputdir, s3_meta_GA, 
                                 s4_meta_GA, scaling_MAD_white, 
                                 scaling_MAD_spec):
    """
    Eureka! Optimization Tools: Parametric Sweep
...
# Parametric sweep function
def parametric_sweep_S4(objective_function, bounds_var, eventlabel,
                        last_s3_meta_outputdir, s3_meta, s4_meta_GA,
                        scaling_MAD_white, scaling_MAD_spec):
    """
    Eureka! Optimization Tools: Parametric Sweep
...
def parametric_sweep_xwindow_crop(objective_function, bounds_var, eventlabel,
                                  ev, pixel_wave_min, pixel_wave_max,
                                  s3_meta_GA, s4_meta_GA):
    """
    Eureka! Optimization Tools: Parametric Sweep
    ...
    for var1 in range(bounds_var[0], bounds_var[1] + 1):
        ...
        try:
            x = np.array([var1])
            fitness_value = objective_function(x, eventlabel, ev,
                                               pixel_wave_min,
                                               pixel_wave_max,
                                               s3_meta_GA, s4_meta_GA)
...
def parametric_sweep_ywindow_crop(objective_function, bounds_var, eventlabel,
                                  ev, s3_meta_GA, s4_meta_GA):
    """
    Eureka! Optimization Tools: Parametric Sweep
...
def parametric_sweep_double(objective_function, bounds_var1, bounds_var2,
                            eventlabel, last_s2_meta_outputdir, s3_meta_GA, 
...
def parametric_sweep_odd_s1(objective_function, bounds_var, eventlabel, 
                            s1_meta_GA, s2_meta_GA, s3_meta_GA, s4_meta_GA, 
                            scaling_MAD_white, scaling_MAD_spec):
    """
    ...
    for var1 in range(bounds_var[0], bounds_var[1] + 1):
        ...
        try:
            x = np.array([var1])
            fitness_value = objective_function(x, eventlabel, s1_meta_GA,
                                               s2_meta_GA, s3_meta_GA,
                                               s4_meta_GA,
                                               scaling_MAD_white,
                                               scaling_MAD_spec)
...
# Parametric sweep function for odd numbers within bounds
def parametric_sweep_odd(objective_function, bounds_var, eventlabel,
                         last_s2_meta_outputdir, s3_meta_GA, s4_meta_GA, 
                         scaling_MAD_white, scaling_MAD_spec):
    """
    Eureka! Optimization Tools: Parametric Sweep for Odd Numbers
...
def parametric_sweep_dqmask(objective_function, bounds_dqmask, eventlabel,
                            last_s2_meta_outputdir, s3_meta_GA, s4_meta_GA, 
                            scaling_MAD_white, scaling_MAD_spec):
    
    best_fitness = float('inf')
    best_params = None
    valid_fitness_scores = []  # Store valid (dqmask_value, fitness_value) pairs

    for dqmask_value in bounds_dqmask:
        try:
            print(f"Testing dqmask={dqmask_value}")
            
            # Pass dqmask_value as the optimization parameter to objective_function
            ...
            print(f"Could not calculate fitness score for dqmask={dqmask_value}. Error: {e}")
            continue

    # If valid fitness scores were found, return the best; otherwise, return None
...
def parametric_sweep_bg_method_s1(objective_function, methods_bg, eventlabel,
                                  s1_meta_GA, s2_meta_GA, s3_meta_GA, 
                                  s4_meta_GA,
                                  scaling_MAD_white, scaling_MAD_spec):
    """
    ...
    Description:
    -----------
    Conducts an exhaustive search over a list of possible string values for `bg_method`
    to minimize the objective function. It evaluates each option in `methods_bg` to
    find the one that produces the lowest fitness score.
    ...
    methods_bg : list of str
        A list containing possible values for the `bg_method` variable. This can
        include any number or type of strings.
    ...
    Notes:
    -----
    If there is an error in calculating the fitness for a particular `bg_method`
    value, it will print an error message and skip that value. This ensures
    robustness in the face of potentially problematic configurations.
    ...
    for i in range(len(methods_bg)):
        ...
        try:
            # Pass bg_method_value as the optimization parameter to objective_function
            ...
            print(f"Could not calculate fitness score for bg_method={bg_method_value}. Error: {e}")
            continue
...
# Parametric Sweep Function for bg_method Evaluation with Dynamic List Handling
def parametric_sweep_bg_method(objective_function, methods_bg, eventlabel,
                                  last_s2_meta_outputdir, s3_meta_GA, s4_meta_GA,
                                  scaling_MAD_white, scaling_MAD_spec):
    """
    Eureka! Optimization Tools: Parametric Sweep for bg_method Evaluation
...
#                       s3_meta_GA, s4_meta_GA, scaling_MAD_white, scaling_MAD_spec, scaling_chi2red,
#                       target_fitness=None):
#     """
...
#         plt.xticks(range(1, len(best_fitness_values) + 1))  # Set x-axis ticks
#         # as integers
...
#         offspring = mutation(offspring, min_bounds, max_bounds, mutation_rate)
...
#         # best_fitness_values.append(fitness_values[np.argmin(fitness_values)])
...
#     """
#     Eureka! Optimization Tools: Crossover Operation for Genetic Algorithm (GA)

#     Description:
#     -----------
#     Performs the crossover operation on pairs of parents to produce offspring.
#     The crossover operation is a crucial step in genetic algorithms, where two
...
#     ----------
#     parents : ndarray
#         A two-dimensional array where each row represents a parent. The number
#         of columns corresponds to the number of genes in each parent.

#     offspring_size : tuple (int, int)
#         A tuple representing the desired shape (number of offsprings, number of
...
#         number of columns corresponds to the number of genes in each offspring.
...
#       single-point crossover, where genes before the crossover point are taken
...
#     Description:
#     -----------
#     Generates an initial population for the genetic algorithm. The individuals
#     in this population are randomly generated based on specified bounds for
#     each gene.
...
#         columns correspond to genes of the individual. The values are randomly
#         generated based on the given bounds.
...
#     """
#     Eureka! Optimization Tools: Selection Mechanism for Genetic Algorithm (GA)
...
#     num_parents : int
#         The number of top-performing individuals to select from the population.
...
#     Description:
#     -----------
#     Selects the best individual (with the lowest fitness score) from a list of
...
#         A list of individuals where each individual is a one-dimensional array
