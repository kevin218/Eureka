...
def source_pos_FWM(flux, meta, m, n=0, plot=True):
    ...
    y_pixels = np.arange(0, x_dim)[max(0,pos_max-meta.spec_hw):min(flux.shape[0],pos_max+meta.spec_hw)]

    sum_row = np.ma.sum(flux, axis=1)[max(0,pos_max-meta.spec_hw):
                                      min(flux.shape[0],pos_max+meta.spec_hw)]
...
def source_pos_gauss(flux, meta, m, n=0, plot=True):
    ...
    pos_max = source_pos_median(flux, meta, m, n=n, plot=False)
    y_pixels = np.arange(0, x_dim)[max(0,pos_max-meta.spec_hw):min(flux.shape[0],pos_max+meta.spec_hw)]
    med_row = np.ma.median(flux, axis=1)[max(0,pos_max-meta.spec_hw):
                                         min(flux.shape[0],pos_max+meta.spec_hw)]
