...
- Stage 2: An optional step that further calibrates Stage 1 data (performs flat-fielding, unit conversion, etc. for JWST observations). This step can be skipped if you'd rather use the ``jwst`` pipeline's Stage 2 outputs, although at present it is recommended that you skip the photom step in that pipeline.
