...
"""
Eureka! Optimization for JWST NIRSpec G395H Data
---------------------------------------------

Description:
This script is designed to optimize the stage 1-4 ECF parameters for JWST NIRSpec G395H observations. 

Inputs:
- Loaded from an input text file, 'optimizer_inputs_nirspec_G395H.txt'.
...
if __name__ == "__main__":
    ...
    ## Load optimizer parameters from input .txt file
    optimizer_params = optimizers.read_inputs("optimizer_inputs_nirspec_G395H.txt")

    # Set each parameter as a local variable (Explicit)
    eventlabel = optimizer_params["eventlabel"]
    nrs = optimizer_params["nrs"]
    subarray = optimizer_params["subarray"]
    xwindow_selection = optimizer_params["xwindow_selection"]
    ywindow_selection = optimizer_params["ywindow_selection"]
    spec_hw_selection = "auto"
    bg_hw_selection = "auto"
    ...
    if bounds_jump_rejection_threshold_s1 == "auto":
        bounds_jump_rejection_threshold_s1 = [4, 15]

    bounds_bg_deg_s1 = optimizer_params["bounds_bg_deg_s1"]
    if bounds_bg_deg_s1 == "auto":
        bounds_bg_deg_s1 = [
            0,
            1,
        ]

    bounds_bg_method_s1 = optimizer_params["bounds_bg_method_s1"]
    if bounds_bg_method_s1 == "auto":
        bounds_bg_method_s1 = [
            "std",
            "mean",
            "median",
    ...
    bounds_p3thresh_s1 = optimizer_params["bounds_p3thresh_s1"]
    if bounds_p3thresh_s1 == "auto":
        bounds_p3thresh_s1 = [3, 7]

    bounds_window_len_s1 = optimizer_params["bounds_window_len_s1"]
    if bounds_window_len_s1 == "auto":
        bounds_window_len_s1 = [1, 21]

    bounds_expand_mask_s1 = optimizer_params["bounds_expand_mask_s1"]
    if bounds_expand_mask_s1 == "auto":
        bounds_expand_mask_s1 = [6, 10]
    ...
    min_bounds = np.array(
        [
            ...
            bounds_bg_deg_s1[0],
            bounds_bg_method_s1[0],
            bounds_p3thresh_s1[0],
            bounds_window_len_s1[0],
            bounds_expand_mask_s1[0],
            bounds_dqmask[0],
            bounds_expand[0],
            bounds_bg_thresh[0],
            bounds_bg_hw[0],
    ...
    max_bounds = np.array(
        [
            ...
            bounds_bg_deg_s1[1],
            bounds_bg_method_s1[1],
            bounds_p3thresh_s1[1],
            bounds_window_len_s1[1],
            bounds_expand_mask_s1[1],
            bounds_dqmask[1],
            bounds_expand[1],
            bounds_bg_thresh[1],
            bounds_bg_hw[1],
    ...
    if xwindow_selection == "auto" and nrs == 1 and subarray == 2048:
        s3_meta_GA.xwindow = [500, 2042]  # NIRSPEC-NRS1
    elif xwindow_selection == "auto" and nrs == 2 and subarray == 2048:
        s3_meta_GA.xwindow = [5, 2028]  # NIRSPEC-NRS2
    elif xwindow_selection == "auto" and subarray == 1024:
        s3_meta_GA.xwindow = [0, 1024]  # NIRSPEC-SUB512
    elif xwindow_selection == "auto" and subarray == 512:
        s3_meta_GA.xwindow = [0, 512]  # NIRSPEC-SUB512

    best["xwindow_LB"] = s3_meta_GA.xwindow[0]
    best["xwindow_UB"] = s3_meta_GA.xwindow[1]

    if ywindow_selection == "auto":
        s3_meta_GA.ywindow = [0, 32]  # NIRPSEC
    ...
    if spec_hw_selection == "auto":
        s3_meta_GA.spec_hw = 4  # # NIRSPEC-G395H
    if bg_hw_selection == "auto":
        s3_meta_GA.bg_hw = 4

    if nrs == 1:
        s1_meta_GA.ignore_low = 650
        s1_meta_GA.ignore_hi = None

    if nrs == 2:
        s1_meta_GA.ignore_low = 0
        s1_meta_GA.ignore_hi = None
    ...
    ## STEP 1.0 - Parametric Sweep of "bg_deg_s1" ##
    ...
    if optimizer == "parametric" and __name__ == "__main__":
        ...
        best["bg_deg_s1"] = best_param[0]

        print("Best parameters: ", best_param[0])
        print("Best fitness: ", best_fitness_value)

        history_fitness_score["bg_deg_s1"] = best_fitness_value

    ## STEP 1.0 - Parametric Sweep of "bg_method_s1" ##
    ...
    #     # Stage 1
    #     s1_meta_GA.jump_rejection_threshold = best["jump_rejection_threshold_s1"]
    #     s1_meta_GA.bg_deg = best["bg_deg_s1"]

    #     # Perform parametric sweep
    #     best_param, best_fitness_value = optimizers.parametric_sweep_bg_method_s1(
    #         objective_funcs.bg_method_s1,
    #         bounds_bg_method_s1,
    #         eventlabel,
    #         s1_meta_GA,
    #         s2_meta_GA,
    ...
    #     best["bg_method_s1"] = best_param

    #     print("Best parameters: ", best_param)
    #     print("Best fitness: ", best_fitness_value)

    #     history_fitness_score["bg_method_s1"] = best_fitness_value

        ### Fix to 'median' to deal with current "waterfall" issue in G395H ###
        best["bg_method_s1"] = 'median'


    ## STEP 1.0 - Parametric Sweep of "p3thresh_s1" ##
    ...
    if optimizer == "parametric" and __name__ == "__main__":
        ...
        s1_meta_GA.jump_rejection_threshold = best["jump_rejection_threshold_s1"]
        s1_meta_GA.bg_deg = best["bg_deg_s1"]
        s1_meta_GA.bg_method = best["bg_method_s1"]

        # Perform parametric sweep
        best_param, best_fitness_value = optimizers.parametric_sweep_S1(
            objective_funcs.p3thresh_s1,
            bounds_p3thresh_s1,
            eventlabel,
            s1_meta_GA,
            s2_meta_GA,
        ...
        best["p3thresh_s1"] = best_param[0]

        print("Best parameters: ", best_param[0])
        print("Best fitness: ", best_fitness_value)

        history_fitness_score["p3thresh_s1"] = best_fitness_value

    ## STEP 1.0 - Parametric Sweep of "window_len_s1" ##
    ...
    if optimizer == "parametric" and __name__ == "__main__":
        ...
        s1_meta_GA.bg_deg = best["bg_deg_s1"]
        s1_meta_GA.bg_method = best["bg_method_s1"]
        s1_meta_GA.p3thresh = best["p3thresh_s1"]

        # Perform parametric sweep for odd numbers
        best_param, best_fitness_value = optimizers.parametric_sweep_odd_s1(
            objective_funcs.window_len_s1,
            bounds_window_len_s1,
            eventlabel,
            s1_meta_GA,
            s2_meta_GA,
            s3_meta_GA,
        ...
        best["window_len_s1"] = best_param[0]

        print("Best parameters: ", best_param[0])
        print("Best fitness: ", best_fitness_value)

        history_fitness_score["window_len_s1"] = best_fitness_value

    ## STEP 1.0 - Parametric Sweep of "expand_mask_s1" ##
    ...
    if optimizer == "parametric" and __name__ == "__main__":
        ...
        s1_meta_GA.bg_method = best["bg_method_s1"]
        s1_meta_GA.p3thresh = best["p3thresh_s1"]
        s1_meta_GA.window_len = best["window_len_s1"]

        # Perform parametric sweep
        best_param, best_fitness_value = optimizers.parametric_sweep_S1(
            objective_funcs.expand_mask_s1,
            bounds_expand_mask_s1,
            eventlabel,
            s1_meta_GA,
            s2_meta_GA,
            s3_meta_GA,
        ...
        best["expand_mask_s1"] = best_param[0]

        print("Best parameters: ", best_param[0])
        print("Best fitness: ", best_fitness_value)

        history_fitness_score["expand_mask_s1"] = best_fitness_value

    ## STEP 1.0 - Run Stages 1 and 2 once and store Stage 2 directory 
    #  to avoid re-running Stages 1 and 2 during Stage 3 optimization (Saves Time)
    ...
    if optimizer == "parametric" and __name__ == "__main__":
        ...
        s1_meta_GA.p3thresh = best["p3thresh_s1"]
        s1_meta_GA.window_len = best["window_len_s1"]
        s1_meta_GA.expand_mask = best["expand_mask_s1"]
    ...
    # STEP 1.2 - Save best S1, S3 & S4 ECF values and update ECF files

    # Create the optimization output directory if it doesn't exist
    if not os.path.exists(outputdir_optimization):
        os.makedirs(outputdir_optimization)

    # Save the best dictionary to a pickle file
    with open(os.path.join(outputdir_optimization, "best_inputs.pkl"), "wb") as f:
        pickle.dump(best, f)

    # Define ECF file paths for Stages 1, 3, and 4.
    s1_ECF_path = os.path.join(s1_meta.outputdir, f"S1_{eventlabel}.ecf")
    s3_ECF_path = os.path.join(s3_meta.outputdir, f"S3_{eventlabel}.ecf")
    s4_ECF_path = os.path.join(s4_meta.outputdir, f"S4_{eventlabel}.ecf")

    ## Define WriteECF to update lines in the ECF file based on a search expression
    def WriteECF(file, searchExp, replaceExp):
        # Iterate over each line and perform in-place replacement
        for line in fileinput.input(file, inplace=1):
            if line.startswith(searchExp):
                line = line.replace(line, replaceExp)
            sys.stdout.write(line)

    # Loop over each best parameter and update the appropriate ECF file.
    for key, value in best.items():
        # Skip keys that contain 'window'
        if 'window' in key:
            continue

        # If the key ends with _s1, update the S1 ECF file with the key suffix removed.
        if key.endswith('_s1'):
            key_without_suffix = key[:-3]  # Remove the last 3 characters: "_s1"
            print(f"Updating S1 parameter: {key_without_suffix} : {value}")
            WriteECF(s1_ECF_path, key_without_suffix, f"{key_without_suffix}\t\t{value}\n")
        else:
            # Otherwise, update the S3 and S4 ECF files using the key as-is.
            print(f"Updating parameter: {key} : {value}")
            WriteECF(s3_ECF_path, key, f"{key}\t\t{value}\n")
            WriteECF(s4_ECF_path, key, f"{key}\t\t{value}\n")
    ...
    if __name__ == "__main__":
        source_file = "optimizer_inputs_nirspec_G395H.txt"
        destination_directory = outputdir_optimization
        destination_filename = "optimizer_inputs_nirspec_G395H_copy.txt"
    ...
    output_file = os.path.join(outputdir_optimization, "optimization_results.txt")
    ...
    with open(output_file, "w") as f:
        ...
        f.write(f"MAD_WLC = {s4_meta_best_WLC.mad_s4_binned[0]}\n")
    ...
    directories = [(outputdir_WLC_S3, "S3"), (outputdir_WLC_S4, "S4")]
    best_directory_white = os.path.join(outputdir_optimization, "White")

    copy_files_and_subfolders_to_new_directory(directories, best_directory_white)

    # Plot Optimization History
    keys_plot = list(history_fitness_score.keys())
    values_plot = list(history_fitness_score.values())

    if scaling_MAD_white == 1 and scaling_MAD_spec == 0:
        y_label = "MAD Value - White"
    elif scaling_MAD_white == 0 and scaling_MAD_spec == 1:
        y_label = "MAD Value - Spectroscopic"
    else:
        y_label = "Fitness Score"

    plt.figure(100, figsize=(12, 6))
    plt.plot(keys_plot, values_plot, marker="o", linestyle="-")
    plt.xticks(rotation=45, ha="right")
    plt.ylabel(y_label)
    plt.xlabel("ECF Parameter")
    plt.title("Fig 100 - Optimization History")
    plt.grid(True)
    plt.tight_layout()

    filepath_history_plot = os.path.join(outputdir_optimization, "Fig100_OptimizationHistory.png")
