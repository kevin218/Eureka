...
def fitlc(eventlabel, params, ecf_path=None, s4_meta=None, input_meta=None):
    ...
    params : Orbital parameters retrieved from exoMAST catalog are imported
        for automation
    ...
    - April 2022 Kevin Stevenson
        Enabled Astraeus
    - March 2023 Reza Ashtari
        Revised to read in params object for optimization
    - April 2023 Reza Ashtari
        Added HSTRampModel
    '''
    ...
    for spec_hw_val in meta.spec_hw_range:
        for bg_hw_val in meta.bg_hw_range:
            ...
            # params = Parameters(meta.folder, meta.fit_par) #Commented out to
            # import values instead - RA 2023
            ...
            if meta.whitep:
                ...
                # if meta.maskFirstOrbit == True:
                #     # Find every index where the value is 0
                #     zero_indices = np.where(meta.orbitnum == 0)[0]
                #     # Use the zero_indices to replace the values
                #     # in the array with NaN
                #     lc.mask_white.values[zero_indices]

                # mask = lc.mask_white.values

                if meta.maskFirstFrames is True:

                    firstFrames = np.where(meta.framenum == 0)
                    firsts = list(firstFrames[0][1:])
                    mask = np.append(lc.mask_white.values, firsts)

                if meta.maskFirstOrbit is True:

                    # Define the indices where the values are zero
                    zero_indices = np.where(meta.orbitnum == 0)

                    # Create a new mask that includes both the
                    # original mask and the zero indices
                    mask_0 = np.zeros_like(lc.mask_white.values)
                    mask_0[zero_indices] = 1
                    mask = np.logical_or(mask_0, lc.mask_white.values)
                ...
                #     # Find every index where the value is 0
                #     zero_indices = np.where(meta.orbitnum == 0)[0]
                #     # Use the zero_indices to replace the values in the
                #     # array with NaN
                #     flux[zero_indices] = np.nan
                #     flux_err[zero_indices] = np.nan
                #     time[zero_indices] = np.nan
                #     # lc[zero_indices] = np.nan
                #     # meta.orbitnum[zero_indices] = np.nan
                #     # mask.extend(zero_indices)
                #     # mask = np.append(mask, zero_indices)
                #     # lc.mask_white.values =
                #     # np.append(lc.mask_white.values, zero_indices)
            ...
            # Now fit the multi-wavelength light curves
                ...
                for channel in range(chanrng):
                    ...
                    if meta.maskFirstFrames is True:
                        ...
                        firstFrames = np.where(meta.framenum == 0)
                        firsts = list(firstFrames[0][1:])
                        mask = np.append(lc.mask.values[channel, :], firsts)
                    ...
                    if meta.maskFirstOrbit is True:
                        ...
                        # Create a new mask that includes both the
                        # original mask and the zero indices
                        mask_0 = np.zeros_like(lc.mask.values[channel, :])
                        mask_0[zero_indices] = 1
                        # mask =
                        # np.logical_or(mask_0, lc.mask.values[channel, :])
                        mask_values = lc.mask.values[channel, :]
                        mask = np.logical_or(mask_0, mask_values)
