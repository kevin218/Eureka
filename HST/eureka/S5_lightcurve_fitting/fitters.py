...
def lsqfitter(lc, model, meta, log, calling_function='lsq', **kwargs):
    ...
    **kwargs : dict
    ...
    - April 10, 2023 Reza Ashtari
        Save the chi2red values within the meta so the genetic
        algorithm can use the chi2red values to assess fitness
    - April 10, 2024 Reza Ashtari
        Adding divide_white
    - July 25, 2024 Reza Ashtari
        Adding divide_white w/ multSpec
    """

    if meta.divide_white is True and meta.multwhite is False:

        # Load the ECSV file
        data_table = ascii.read(meta.residuals_white_dir, format='ecsv')

        # Extract the 'lcdata' column
        lcdata_white = data_table['lcdata'].data

        # Extract the 'model' column
        model_white = data_table['model'].data

        systematics_white = lcdata_white / model_white
        lc.flux /= systematics_white

    if meta.divide_white and meta.multwhite:

        systematics_white = []

        if meta.residuals_white_dir_list is None:
            raise ValueError("residuals_white_dir_list cannot be None")

        for i in range(len(meta.residuals_white_dir_list)):
            # Load the ECSV file
            data_table = ascii.read(meta.residuals_white_dir_list[i], format='ecsv')
            ...
            # Calculate the Systematic White Noise
            systematics_white_perTransit = lcdata_white / model_white

            # Append Results
            systematics_white.extend(systematics_white_perTransit)

        # Convert to numpy array
        systematics_white = np.array(systematics_white)

        # Ensure the shape matches lc.flux
        if systematics_white.shape[0] != lc.flux.shape[0]:
            raise ValueError("Shape mismatch: systematics_white and lc.flux must have the same length")
...
def emceefitter(lc, model, meta, log, **kwargs):
    ...
    **kwargs : dict
    ...
    # if meta.divide_white is True:

    #     if meta.multwhite is None or meta.multwhite is False:

    #         residuals_white_dir = meta.residuals_white_dir

    #         # Load the ECSV file
    #         data_table = ascii.read(residuals_white_dir, format='ecsv')
