        ...
        for bg_hw_val in meta.bg_hw_range:
            ...
            if hasattr(meta, 'manual_clip') and meta.manual_clip is not None:
                ...
                if meta.multwhite:
                    ...
                    for p in range(len(meta.inputdirlist)+1):
                        meta, lc_specs[p], log = \
                            util.manual_clip(lc_specs[p], meta, log)
                        meta.nints[p] = len(lc_specs[p].time.values)
                else:
            ...
            if meta.use_generate_ld:
                ...
                if meta.multwhite:
                    ...
                    nchan = len(meta.inputdirlist) + 1
                    lin_c1 = np.zeros((nchan, 1))
                    quad = np.zeros((nchan, 2))
                    nonlin_3 = np.zeros((nchan, 3))
                    nonlin_4 = np.zeros((nchan, 4))
                    # Load LD coefficient from each lc
                    for p in range(nchan):
                        if not hasattr(lc_specs[p], ld_str + '_lin'):
                            raise Exception("Exotic-ld coefficients have not" +
                                            " been calculated in Stage 4")
                        log.writelog("\nUsing generated limb-darkening " +
                                     f"coefficients with {ld_str} \n")
                        lin_c1[p] = lc_specs[p][ld_str + '_lin'].values[0]
                        quad[p] = lc_specs[p][ld_str + '_quad'].values[0]
                        nonlin_3[p] = lc_specs[p][ld_str + '_nonlin_3para'] \
                            .values[0]
                        nonlin_4[p] = lc_specs[p][ld_str + '_nonlin_4para'] \
                            .values[0]
                    ld_coeffs = [lin_c1, quad, nonlin_3, nonlin_4]
                else:
                    if not hasattr(lc, ld_str + '_lin'):
            ...
            if meta.multwhite:
                ...
                me.saveevent(meta, (meta.outputdir+'S5_'+meta.eventlabel +
                                    "_Meta_Save"), save=[])  
            # Joint Spectral Channel Fits (may have different time axis)
            elif meta.multspec:
                log.writelog("\nStarting Shared Fit of Spectral Light Curves\n")
                ...
                for pi in range(len(meta.inputdirlist)+1):
                    mask = lc_specs[pi].mask.values[0, :]
                    flux_temp = np.ma.masked_where(
                        mask, lc_specs[pi].data.values[0, :])
                    err_temp = np.ma.masked_where(
                        mask, lc_specs[pi].err.values[0, :])
                    flux_temp, err_temp = util.normalize_spectrum(
                        meta, flux_temp, err_temp,
                        scandir=getattr(lc_specs[pi], 'scandir', None))
                    time_temp = lc_specs[pi].time.values - offset
                    flux = np.ma.append(flux, flux_temp)
                    flux_err = np.ma.append(flux_err, err_temp)
                    time = np.ma.append(time, time_temp)

                    if hasattr(lc_specs[pi], 'centroid_x'):
                        xpos_temp = np.ma.masked_invalid(
                            lc_specs[pi].centroid_x.values)
                    else:
                        xpos_temp = None
                    if hasattr(lc_specs[pi], 'centroid_y'):
                        ypos_temp = np.ma.masked_invalid(
                            lc_specs[pi].centroid_y.values)
                    else:
                        ypos_temp = None

                    if hasattr(lc_specs[pi], 'centroid_sx'):
                        xwidth_temp = np.ma.masked_invalid(
                            lc_specs[pi].centroid_sx.values)
                    else:
                        xwidth_temp = None
                    if hasattr(lc_specs[pi], 'centroid_sy'):
                        ywidth_temp = np.ma.masked_invalid(
                            lc_specs[pi].centroid_sy.values)
                    else:
                        ywidth_temp = None
            ...
            elif meta.sharedp and not meta.multwhite and not meta.multspec:  # RA Edit - March 2024
...
def make_longparamlist(meta, params, chanrng):
    ...
    # elif meta.sharedp and not meta.multwhite:
    elif meta.sharedp and not meta.multwhite and not meta.multspec:
        nspecchan = chanrng
    else:
        nspecchan = 1
