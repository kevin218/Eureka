...
def genlc(eventlabel, ecf_path=None, s3_meta=None, input_meta=None):
    ...
    for spec_hw_val in meta.spec_hw_range:
        for bg_hw_val in meta.bg_hw_range:
            ...
            # Perform manual clipping if required
            if hasattr(meta, 'manual_clip') and meta.manual_clip is not None:
                meta, lc, log = util.manual_clip_s4(lc, meta, log)
                # Recompute time_inds if not directly returned from manual_clip
                time_inds = np.where(np.isin(spec['time'].values, lc['time'].values))[0]

            # Apply clipping to all other time-dependent datasets in lc
            time_dependent_keys = ['centroid_y', 'centroid_sy', 'centroid_x', 'centroid_sx', 'scandir']
            for key in time_dependent_keys:
                if hasattr(spec, key):
                    # Clip each array to match the length of lc['time']
                    lc[key] = spec[key][time_inds]

            # Ensure any new arrays added to 'lc' match the new length of the 'time' dimension
            if meta.recordDrift or meta.correctDrift:
                lc['centroid_x'] = (['time'], np.zeros(len(lc['time'])))
            ...
            if meta.recordDrift or meta.correctDrift:
                ...
                # apply manual clipping to drift1d
                drift1d = drift1d[:len(lc['centroid_x'])]
