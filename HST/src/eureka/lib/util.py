...
def manual_clip(lc, meta, log):
    ...
    # # Remove the requested integrations from lc
    # lc = lc.isel(time=time_inds)

    # # Adjusting for scandir
    # if hasattr(meta, 'scandir'):
    #     # Create a new boolean array for integrations
    #     integration_bool = np.ones(len(meta.scandir), dtype=bool)
    #     for inds in meta.manual_clip:
    #         # Convert time indices to integration indices
    #         start_integration = inds[0] // meta.nreads
    #         end_integration = inds[1] // meta.nreads
    #         integration_bool[start_integration:end_integration] = False

    #     # Apply this integration-level boolean array to meta.scandir
    #     meta.scandir = meta.scandir[integration_bool]

    # return meta, lc, log


def manual_clip_s4(lc, meta, log):
    """Manually clip integrations along time axis.
    ...
    log : logedit.Logedit
        The updated log.
    """
    log.writelog('Manually removing data points from meta.manual_clip...', mute=(not meta.verbose))

    meta.manual_clip = np.array(meta.manual_clip)
    if len(meta.manual_clip.shape) == 1:
        meta.manual_clip = meta.manual_clip[np.newaxis]

    # Calculate the boolean mask for clipping
    time_bool = np.ones(len(lc.data.time), dtype=bool)
    for inds in meta.manual_clip:
        start_index = max(0, inds[0])
        end_index = min(inds[1], len(lc.data.time))
        time_bool[start_index:end_index] = False
    time_inds = np.nonzero(time_bool)[0]

    # Clip each variable in lc that depends on the 'time' dimension
    for var in lc.variables:
        if 'time' in lc[var].dims:
            max_valid_index = min(max(time_inds), len(lc[var].data) - 1)
            valid_time_inds = time_inds[time_inds <= max_valid_index]
            lc[var] = lc[var].isel(time=valid_time_inds)

    # Adjusting for scandir, if necessary
    if hasattr(meta, 'scandir'):
        integration_bool = np.ones(len(meta.scandir), dtype=bool)
        for inds in meta.manual_clip:
            start_integration = max(0, inds[0] // meta.nreads)
            end_integration = min(inds[1] // meta.nreads, len(meta.scandir))
            integration_bool[start_integration:end_integration] = False
        meta.scandir = meta.scandir[integration_bool]
