...
def Stage4(optimizer_inputs_file, loc_sci_list, filepath_best_inputs_list):
    ...
    for i in range(num_transits):
        ...
        if optimizer_params['skip_xwindow_crop'] is False:
        ...
        if optimizer_params['skip_ywindow_crop'] is False:
        ...
        increment = 0.02
        num_channels = 27
        # wave_min_start = 1.12  # RA Edit - August 2024
        # wave_max_start = 1.13
        # increment = 0.01
        # num_channels = 55
...
def Stage5(eventlabel, run_dir, transits_to_mask):
    ...
    for c in range(matching_chan_count):
        ...
        pattern_to_find = r'ap(\d{2}|\d{3})_bg(\d{2}|\d{3})'
